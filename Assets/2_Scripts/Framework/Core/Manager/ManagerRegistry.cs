...
namespace YGZFrameWork
{
    ...
    public static class ManagerRegistry
    {
        ...
        public static readonly (Type Type, int Priority, Func<object> GetInstance)[] Entries = new (Type, int, Func<object>)[]
        {
            ...
            (typeof(ResourceManager),  2, () => (object)ResourceManager.Instance), // 资源加载器，UIManager/CfgToolManager 依赖
            (typeof(CanvasManager),    3, () => (object)CanvasManager.Instance),    // Canvas 层级构建，UIManager 依赖它
            (typeof(UIManager),        4, () => (object)UIManager.Instance),      // UI 面板系统，依赖 CanvasManager + ResourceManager

            // 业务逻辑层
            (typeof(TimeManager),      5, () => (object)TimeManager.Instance),     // 时间同步
            (typeof(NetWorkManager),   6, () => (object)NetWorkManager.Instance), // 网络请求
            (typeof(DiceManager),      7, () => (object)DiceManager.Instance),    // 骰子游戏逻辑
            (typeof(ThreadManager),    8, () => (object)ThreadManager.Instance),   // 后台任务
